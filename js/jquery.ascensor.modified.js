        ...
        if (self.options.queued) {
          ...
          if (self.options.queuedDirection === "x") {
            ...
            if ($(node).scrollTop() === self.options.ascensorMap[floor][0] * WH) {
              ...
              {
              start: function() {
              	$("#ascensorFloor" + floorActive + " .page-wrapper").animate({"opacity":0}, 300);
              },
              duration: time,
              easing: self.options.easing, complete: function() {
                scrollEnd();
              	$("#ascensorFloor" + floorActive + " .page-wrapper").animate({"opacity":1}, 300);
              	if(!$("#ascensorFloor" + floorActive).hasClass("projects_Floor")) {
              		$("#ascensorFloor" + floorActive + " h2").shuffleLetters({'fps':50});
  					$("#ascensorFloor" + floorActive + " p").shuffleLetters({'fps':250});
  				}
  				$(".ascensorFloor:not([id='ascensorFloor" + floorActive + "']) .page-wrapper").css("opacity",0);
  				...
  				if(floorActive == 32) {
    				...
    				$("header").addClass("noscroll");
  					...
  					$("article").each(function(index, elm) {
	  					if(!$(elm).hasClass($("#ascensorFloor" + floorActive).attr("class").split(" ")[0])) {
              ...
              complete: function() {
                ...
                easing: self.options.easing, complete: function() {
                  scrollEnd();
                  $("#ascensorFloor" + floorActive + " .page-wrapper").animate({"opacity":1}, 700);
                  if(!$("#ascensorFloor" + floorActive).hasClass("projects_Floor")) {
                  	$("#ascensorFloor" + floorActive + " h2").shuffleLetters({'fps':50});
  					$("#ascensorFloor" + floorActive + " p").shuffleLetters({'fps':250});
                  }
